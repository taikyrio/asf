...
const windows = {
    ...
    playerData: {
        ...
        criminalRecord() {
            const { yearsInPrison, murderAttempts, murder, prisonEscapes } =
    ...
    socialMedia: {
            ...
            recordVideo: {
                ...
                record() {
                    const title = document
                        ...
                        .value.trim();
                    const thematic =
                        document.getElementById("thematic-selector").value;

                    if (title === "")
                        return windows.socialMedia.youtube.recordVideo.display();
                    const subscribers = player.socialMedia.youtube.subscribers;
                    const views = Math.floor(
                        Math.random() * (subscribers / 5) + 5 + subscribers,
                    );
        ...
        instagram: {
            display() {
                const isBanned = countryQuery(player.location).laws
                    .banned_instagram;
            ...
            photo: {
                ...
                upload() {
                    ...
                    const newFollowers =
                        Math.round(
                            (Math.random() * 3) / followers != 0
                                ? followers
                                : 2,
                        ) + player.stats.appearance;
                    const likes = Math.round(
                        Math.random() *
                            (followers * (player.stats.appearance / 100)),
                    );
    ...
    relations: {
        display(target) {
            ...
            const person =
                player.relationships[personCategory][parseInt(personID)];
        ...
        meanOptions(data) {
            ...
            ${
                player.age > 14
                    ? `<div class="option" onclick="windows.mean.relations.assault(this)" data-index="${index}">Assault</div>`
                    : ""
            }
            ${
                person.relationships.partner[0].characterIndex ===
        ...
        mean: {
            yell(data) {
                ...
                if (isPartner) person.stats.loveToPartner -= 20;
                statsLimit(person);
            ...
            insult(data) {
                ...
                if (isPartner) person.stats.loveToPartner -= 30;
        ...
        romance: {
            ...
            break() {
                ...
                exPartner.stats.relationWithPlayer -=
            ...
            sex(useProtection) {
                ...
                if (!useProtection) {
                    ...
                    if (
                        ...
                        (player.gender == "female" && partner.gender == "male")
                    ) {
                        if (player.gender == "female") player.pregnant = true;
    ...
    love: {
        findLove() {
            ...
            let possiblePartner = new Person(
                undefined,
                undefined,
                player.age,
                targetGender[player.sexuality],
                undefined,
                0,
                player.location,
            );
            ...
            if (possiblePartner.age >= 18) jobAssigner(possiblePartner);
        ...
        tryPartner() {
            ...
            if (random + appearance > 100) {
                possiblePartner.stats.relationWithPlayer =
                    50 + Math.floor(Math.random() * 50);
                possiblePartner.stats.loveToPartner =
    ...
    university: {
        ...
        chooseCareer(payer, paidBy) {
            ...
            for (let option of document.getElementsByClassName("option")) {
                option.addEventListener("click", (e) => {
                    ...
                    if (decision === "yes") {
                        const chosenCareer =
                            document.getElementById("career-selector").value;
                        player.currentCareer = Object.assign(
                            { studying: true },
                            universityCareers[chosenCareer],
                        );
        ...
        paidByParents() {
            ...
            if (
                ...
                windows.university.chooseCareer(
                    dad.alive && dad.money.income - dad.money.expenses >= 6000
                        ? dad
                        : mom,
                    "parents",
            ...
            for (let requirement of Object.entries(requirements)) {
                const skillVerifier = () => {
                    ...
                    if (skills.includes(requirement[0]))
                        if (
                            player.skills[requirement[0]].level >=
                            requirement[1]
                        )
                ...
                if (
                    requirement[0] === "education" &&
                    Object.entries(player.career).length > 0 &&
                    player.career[requirement[1]] != undefined
                )
                    if (player.career[requirement[1]].label === requirement[1])
                        requirementsCompleted++;
        ...
        handleSwitch(option) {
            ...
            switch (option) {
                case "reading":
                    ...
                    freetime.isReading
                        ? (freetime.isReading = false)
                        : (freetime.isReading = true);
                    freetime.isReading
                ...
                case "parties":
                    ...
                    freetime.isAttendingParties
                        ? (freetime.isAttendingParties = false)
                        : (freetime.isAttendingParties = true);
                    freetime.isAttendingParties
                        ? (button.style.float = "right")
                        : (button.style.float = "left");
                    freetime.isAttendingParties
                ...
                case "musicLessons":
                    ...
                    freetime.isTakingMusicLessons
                        ? (freetime.isTakingMusicLessons = false)
                        : (freetime.isTakingMusicLessons = true);
                    freetime.isTakingMusicLessons
                        ? (button.style.float = "right")
                        : (button.style.float = "left");
                    freetime.isTakingMusicLessons
                ...
                case "gym":
                    ...
                    freetime.goesToGym
                        ? (freetime.goesToGym = false)
                        : (freetime.goesToGym = true);
                    freetime.goesToGym
                        ? (button.style.float = "right")
                        : (button.style.float = "left");
                    freetime.goesToGym
